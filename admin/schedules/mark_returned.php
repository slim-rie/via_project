...
if ($updateDelivery->execute()) {
    // Set the associated truck back to available
    $truckQuery = $con->prepare("UPDATE trucks SET status = 'available' 
                                 WHERE truck_id = (SELECT truck_id FROM schedules WHERE schedule_id = ?)");
